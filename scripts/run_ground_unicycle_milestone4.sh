...
python $ROOT/scripts/smoke_ground_unicycle.py --assets-dir $RUN_DIR/assets --ds-meters 0.02 --max-speed-mps 0.20 --max-accel-mps2 0.30 --max-omega-radps 1.0 --max-alpha-radps2 2.0 --dt 0.02 --settle-timeout 5.0 --kx 1.0 --ky 2.0 --ktheta 2.0
EOF
...
python "$ROOT/scripts/smoke_ground_unicycle.py" \
  ...
  --ds-meters 0.02 --max-speed-mps 0.20 --max-accel-mps2 0.30 \
...
- Diagnostics: inflated-grid consistency and executed-trajectory polygon containment are reported separately because the grid is a conservative seed-search model and the milestone-2 corridor constrains the reference, not a tracking-error tube.
